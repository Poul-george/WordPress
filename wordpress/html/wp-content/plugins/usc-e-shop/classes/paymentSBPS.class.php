...
class SBPS_SETTLEMENT extends SBPS_MAIN {
	...
	public function initialize_data() {
		...
		// if ( ! isset( $options['acting_settings'] ) || ! isset( $options['acting_settings']['sbps'] ) ) {
			...
			$options['acting_settings']['sbps']['auto_settlement_mail'] = ( isset( $options['acting_settings']['welcart']['auto_settlement_mail'] ) ) ? $options['acting_settings']['welcart']['auto_settlement_mail'] : 'off';
...
<sps-api-request id="ST02-00201-311">
	...
	public function get_acting_log( $order_id = 0, $tracking_id = 0, $result = 'OK' ) {
		...
		if ( empty( $order_id ) ) {
			...
			if ( empty( $tracking_id ) ) {
				if ( 'OK' == $result ) {
					$query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}usces_acting_log WHERE `order_id` = %d AND `result` IN ( 'OK', 'AC' ) ORDER BY ID DESC, datetime DESC",
						$order_id
					);
				} else {
					$query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}usces_acting_log WHERE `order_id` = %d ORDER BY ID DESC, datetime DESC",
						$order_id
					);
				}
			} else {
...
<sps-api-request id="ST02-00201-101">
	...
	<sps_hashcode>' . $sps_hashcode . '</sps_hashcode>
				...
				$log = array( 'acting' => $this->acting_card, 'key' => $rand, 'result' => $res_err_code, 'data' => $data );
				usces_save_order_acting_error( $log );
				$this->save_acting_log( $response_settlement, $this->acting_card, $acting_opts['sales_dlseller'], 'NG', $order_id, '' );
...
<sps-api-request id="ST02-00201-101">
	...
	<sps_hashcode>' . $sps_hashcode . '</sps_hashcode>
			...
			$log = array( 'acting' => $this->acting_card . '(member_process)', 'key' => $member_id, 'result' => $cust_ref['result'], 'data' => $cust_ref );
			usces_save_order_acting_error( $log );
	...
	private function set_regular_ba_id( $member_id, $reg_id, $ba_id ) {
		...
		$usces->set_member_meta_value( 'sbps_regular_' . $reg_id, $ba_id, $member_id );
	}
	...
	private function get_regular_ba_id( $member_id, $reg_id = 0 ) {
		...
		if ( empty( $reg_id ) ) {
			...
			$query             = $wpdb->prepare( "SELECT COUNT(*) FROM {$member_meta_table} WHERE `member_id` = %d AND `meta_key` LIKE %s", $member_id, 'sbps_regular_%' );
			$ba_id             = $wpdb->get_var( $query );
		} else {
			$ba_id = $usces->get_member_meta_value( 'sbps_regular_' . $reg_id, $member_id );
		}
		return $ba_id;
	}
