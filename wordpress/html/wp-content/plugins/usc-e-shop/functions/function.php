...
function usces_ajax_send_mail() {
	...
	$attachments = '';
	if ($usces->options['email_attach_feature'] == 1 && isset($_FILES['attachFile'])) {
		$attach_file = $_FILES['attachFile'];
...
function usces_check_validate_attach_file($file) {
	...
	if (count($email_attach_file_extension) > 0 && isset($file['name']) && !empty($file['name'])) {
		...
		if (count($arr_file_name) > 1) {
			$file_extention = strtolower($arr_file_name[count($arr_file_name) - 1]);
