...
function print_google_recaptcha_response($action, $parent_element_id, $form_name = ''){
	...
	if(isset($option['system']['google_recaptcha']['status']) && $option['system']['google_recaptcha']['status'] && !(empty($option['system']['google_recaptcha']['site_key'])) && !(empty($option['system']['google_recaptcha']['secret_key']))){
		echo  '<script src="https://www.google.com/recaptcha/api.js?render=' . $option['system']['google_recaptcha']['site_key'] . '"></script>';
		echo  '<script>';
		echo  'grecaptcha.ready(function() {';
		echo  'grecaptcha.execute(\''. $option['system']['google_recaptcha']['site_key'] .'\', {action: "'. $action.'"}).then(function(token) {';
		if($form_name != ''){
			echo  'jQuery("form[name=\''.$form_name.'\'").append(\'<input type="hidden" name="recaptcha_response" value="\'+ token +\'">\');';
		}
		else{
			echo  'jQuery("#' . $parent_element_id . '").find("form").append(\'<input type="hidden" name="recaptcha_response" value="\'+ token +\'">\');';
		}
		echo  '});';
		echo  '});';
		echo  '</script>';
