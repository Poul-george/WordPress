...
$mailsenddialog_height = ( 1 == $this->options['email_attach_feature'] ) ? 628 : 578;
?>
<style>
	...
	uscesMail = {
		sendmail : function() {
			...
			if (email_attach_feature == 1) {
				if (attachFile.files[0]) {
					...
					if (!confirm('<?php _e('Are you sure you want to send it without attachments?', 'usces'); ?>')) return;
				}
			}
			s.data = formData;
			...
			$.ajax( s ).done(function( data ){
				...
				<?php echo apply_filters( 'usces_filter_order_check_mail_js', '' ); ?>
					...
					$('#mailSendAlert').dialog('option', 'title', 'ERROR');
					$('#mailSendAlert fieldset').html('<p><?php _e('Failure in attach file e-mails.', 'usces'); ?></p>');
					$('#mailSendAlert').dialog('option', 'close', function() {
						$(this).dialog('close');
					});
					$('#mailSendAlert').dialog( "option", "modal", true );
		...
		validateFileAttach: function() {
			...
			if (!checkValidSize || !checkValidExtension) {
				...
				if (!checkValidSize) {
					msgSize = "<p><?php _e('The maximum file size that can be attached has been exceeded.', 'usces'); ?></p>";
				}
