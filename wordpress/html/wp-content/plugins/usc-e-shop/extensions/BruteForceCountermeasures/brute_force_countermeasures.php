...
class USCES_BRUTE_FORCE_COUNTER_MEASURES
{
	...
	public function setting_form()
	{
		...
		<form action="" method="post" name="option_form" id="brute_force_form">
			<div class="postbox">
				<h3 class="hndle" id="brute_force">
					<span><?php _e('Brute-force attack countermeasures', 'usces'); ?></span><?php echo $status; ?></h3>
				<div class="inside">
						...
						<tr height="35">
							...
							<?php foreach (self::$default_option['monitoring_span'] as $value) { ?>
								<td width="10"><input name="monitoring_span" type="radio"
													  id="brute_force_monitoring_span_<?php echo $value; ?>"
													  value="<?php echo $value; ?>"<?php if (self::$opts['monitoring_span'] === $value) echo 'checked="checked"'; ?> />
								</td>
								<td width="100"><label
											for="brute_force_monitoring_span_<?php echo $value; ?>"><?php printf(__("%s minutes", 'usces'), $value); ?></label>
								</td>
							<?php } ?>
							<td>
						...
						<tr height="35">
							...
							<?php foreach (self::$default_option['num_of_errors'] as $value) { ?>
								<td width="10"><input name="num_of_errors" type="radio"
													  id="brute_force_num_of_errors_<?php echo $value; ?>"
													  value="<?php echo $value; ?>"<?php if (self::$opts['num_of_errors'] === $value) echo 'checked="checked"'; ?> />
								</td>
								<td width="100"><label
											for="brute_force_num_of_errors_<?php echo $value; ?>"><?php printf(__("%s times", 'usces'), $value); ?></label>
								</td>
							<?php } ?>
							<td>
						...
						<tr height="35">
							...
							<?php foreach (self::$default_option['rejection_time'] as $value) { ?>
								<td width="10"><input name="rejection_time" type="radio"
													  id="brute_force_rejection_time_<?php echo $value; ?>"
													  value="<?php echo $value; ?>"<?php if (self::$opts['rejection_time'] === $value) echo 'checked="checked"'; ?> />
								</td>
								<td width="100"><label
											for="brute_force_rejection_time_<?php echo $value; ?>"><?php printf(__("%s minutes", 'usces'), $value); ?></label>
								</td>
							<?php } ?>
							<td>
